...
class UberAccessibilityService : AccessibilityService() {
    ...
    private fun performInDriverScan() {
        // Tenta captura de tela direta via ML Kit OCR se Android 11+ (API 30+)
                ...
                object : com.uberanalyzer.ocr.MlKitScreenOcrEngine.OcrResultCallback {
                    override fun onOcrCompleted(
                        ...
                        if (lowerText.contains("r$") || lowerText.contains("oferecer") || lowerText.contains("aceitar") || lowerText.contains("recusar")) {
                            sendDebugLog("👁️ ML Kit OCR: Leitura de imagem/pixels da tela efetuada com sucesso!")
                            processInDriverQueue(extractedText, isOcrSource = true)
                        } else {
                            performFallbackAccessibilityScan()
                        }
                    }
                    ...
                    override fun onError(error: Exception) {
                        // Se falhar o OCR de tela (ex: permissão ou versão), faz fallback automático para nós de acessibilidade
                        performFallbackAccessibilityScan()
                    }
                }
            )
    ...
    private fun processInDriverQueue(text: String, isOcrSource: Boolean = false) {
        ...
        if (capturedRides.isNotEmpty()) {
            ...
            val sourceName = if (isOcrSource) "ML Kit OCR" else "Leitor de Acessibilidade"
            sendDebugLog("📥 Capturadas ${capturedRides.size} corridas via $sourceName na Lista inDriver!")
            ...
            // Se OCR capturou texto mas não gerou corridas, chama o leitor por nó como fallback
            performFallbackAccessibilityScan()
        }
    }
