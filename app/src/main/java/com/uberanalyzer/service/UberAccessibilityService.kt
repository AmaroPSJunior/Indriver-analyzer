...
class UberAccessibilityService : AccessibilityService() {
            ...
            com.uberanalyzer.ocr.MlKitScreenOcrEngine.captureAndProcessScreen(
                ...
                object : com.uberanalyzer.ocr.MlKitScreenOcrEngine.OcrResultCallback {
                    override fun onOcrCompleted(
                        ...
                        if (extractedText.isNotBlank() && (lowerText.contains("r$") || lowerText.contains("oferecer") || lowerText.contains("aceitar") || lowerText.contains("recusar"))) {
                            sendDebugLog("👁️ ML Kit OCR: Pixels da tela lidos com sucesso!")
                            val processed = processInDriverQueue(extractedText, isOcrSource = true)
                            if (!processed) {
    ...
    private fun processInDriverQueue(text: String, isOcrSource: Boolean = false): Boolean {
        ...
        if (capturedRides.isNotEmpty()) {
            ...
            val sourceName = if (isOcrSource) "ML Kit OCR" else "Acessibilidade (Nós)"
