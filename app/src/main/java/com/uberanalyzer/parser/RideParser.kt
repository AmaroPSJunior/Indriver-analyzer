...
object RideParser {
    ...
    private fun isValidAddressLine(line: String): Boolean {
        val lower = line.lowercase(Locale.getDefault()).trim()
        if (lower.length < 3) return false

        // Absolute disqualifiers: prices, rates, ratings, payment methods, durations
        if (line.contains("★") || line.contains("R$") || line.contains("Pix", true) || lower.contains("/km")) return false

        // Disqualify system UI, media, notifications, dates, clock, status bar
        if (lower.contains("spotify") || lower.contains("waze") || lower.contains("sinal") ||
            ...
            lower.contains("de fev") || lower.contains("de mar") || lower.contains("de abr") ||
            lower.contains("sua oferta") || lower.contains("oferecer") || lower.contains("aceitar") || lower.contains("recusar")) return false
        ...
        // Address format with punctuation or number (e.g., Augusta, 100 - Consolação) that has letters and no distance/time tags
        val hasAddressFormat = (lower.contains(",") || lower.contains("-") || lower.contains("nº") || lower.contains("n°")) &&
                lower.any { it.isLetter() } && !lower.contains("min") && !lower.contains("km")

        return (startsWithLogradouro || hasStreetOrLandmark || hasAddressFormat)
    }
