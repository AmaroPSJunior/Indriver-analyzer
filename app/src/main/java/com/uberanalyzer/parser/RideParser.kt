...
object RideParser {
     ...
     * extracted from Google ML Kit OCR.
     */
    fun parseInDriverSpatialLines(lines: List<com.uberanalyzer.ocr.MlKitScreenOcrEngine.OcrLine>): List<InDriverRide> {
        ...
        for (line in validLines) {
            ...
            val isAddressLine = isValidAddressLine(text)
            ...
            // Check if line looks like a passenger name
            ...
            val shouldSplit = (clusterHasPrice && clusterHasAddress && (isPriceOrRate || isNameCandidate)) || 
                              (verticalGap > 110 && currentCluster.isNotEmpty())
        ...
        for (cluster in cardClusters) {
            ...
            // Passenger Name (Amarelo - canto superior esquerdo)
            val passengerName = extractPassengerNameClean(clusterTexts) ?: "Passageiro inDrive"
            val passengerPhoto = extractPassengerPhotoUrl(clusterTexts)

            // Addresses (Laranja - Origem e Destino)
